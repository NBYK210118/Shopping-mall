...
const Mypage = () => {
  ...
  const SideBar = () => {
    const result = items.map((item, idx) => (
      <div
        key={idx}
        className={`w-[80%] h-[10%] text-white border border-solid rounded-xl flex justify-center items-center hover:cursor-pointer shadow-lg ${
          menuStates[item.txt]
  ...
  return (
    <div className="w-[1296px] h-[560px] flex justify-center">
      <div className="w-[95%] h-full flex justify-center items-center relative">
        <div
          id="mypage_content"
          className="w-[90%] h-[90%] flex absolute top-24"
        >

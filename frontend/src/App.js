...
function MainHeader() {
  ...
  const menus = () => {
    ...
    const links = result.map((val, idx) => (
      <div
        key={idx}
        className="rounded-lg block p-3 hover:bg-sky-300 transition-all duration-300"
      >
        <NavLink
          to={val.to}
          onClick={closeMenu}
          className="block text-base font-semibold text-white hover:bg-sky-300"
        >
          {val.text}
        </NavLink>
      </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          {token ? (
            <>
              <div className="border border-transparent rounded-lg flex p-4 hover:bg-sky-300 hover:cursor-pointer transition-all duration-400">
                <NavLink
                  key="2"
                  to="/user/my-profile"
                  onClick={closeMenu}
